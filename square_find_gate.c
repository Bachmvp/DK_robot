...
int gate_loose(double speed, int time) {
	...
	{
	case 0:
		if (follow_line(speed / 2, 'r', 0, mission.sub_time, sens.cross)) {
			mission.sub_state = 1;
			mission.sub_time = (-1);
			dist_to_wall = 0.9;
			mot.flag_collision=0£»
		...
		break;
	case 1:
		if (follow_line(speed / 2, 'c', 0, mission.sub_time, laserpar[0] < 0.30)) {
		...
		break;
	case 2:
		if (follow_line(speed / 2, 'c', 0, mission.sub_time, laserpar[0] >0.30)) {//go forward no longer see gatepost
		...
		break;
	case 3:
		if (follow_line(speed / 2, 'c', 0, mission.sub_time, laserpar[0] < 0.30)) {//go forward see gatepost
			printf("gate!! %f\n", laserpar[0]);
			mission.sub_state = 4;
			mission.sub_time = (-1);
		}
