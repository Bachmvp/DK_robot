...
const double BLACK_THRESHOLD = 0.24;
...
{    case 0:
      ...
      break;
     case 1:
         if (follow_line(speed/4,'c',1,mission.sub_time, sens.w_cross,followdist)){
     ...
     break;
     case 3:
         if (fwd(0.3,speed/2,mission.sub_time,sens.cross)){
         mission.sub_state = 2;
         mission.sub_time = (-1);}
      /*case 9:
...
int goto_box(double speed,double followdist) {
	...
	case 0:
		...
		break;
        case 1:
		if (follow_line(speed/2, 'c', 0, mission.sub_time, sens.ir_min<0.1, followdist)) {
			mission.sub_state = 2;
			mission.sub_time = (-1);
		...
		break;
	case 2:
		if (fwd(0.5+0.4, 20, mission.sub_time, 0)) {
			mission.sub_state = 3;
			mission.sub_time = (-1);
		}
		break;
...
int gate_loose(double speed, int time,double followdist) {
	...
	{
	case 0:
		if (follow_line(speed / 3, 'c', 0, mission.sub_time, sens.cross,followdist)) {
			mission.sub_state = 1;
			mission.sub_time = (-1);
			dist_to_wall = 1.4;
			mot.flag_collision=0;
		...
		break;
	case 1:
		if (follow_line(speed / 3, 'c', 0, mission.sub_time, (laserpar[8] < (dist_to_wall-0.3))&&(laserpar[8]!=0),followdist)) {
		...
		break;
	case 2:
		if (follow_line(speed / 4, 'c', 0, mission.sub_time, (laserpar[8] >(dist_to_wall-0.3))&&(laserpar[8]!=0),followdist)) {//go forward no longer see gatepost
		...
		break;
	case 3:
		if (follow_line(speed / 4, 'c', 0, mission.sub_time, (laserpar[8] < (dist_to_wall-0.3))&&(laserpar[8]!=0),followdist)) {//go forward see gatepost
			printf("gate!! %f\n", laserpar[8]);
			mission.sub_state = 4;
			mission.sub_time = (-1);
		}
...
int gate_wall(double speed,double dist_wall, int time) {
	...
	{
	case 0:
		if (follow_wall(speed, 'l', dist_wall, mission.sub_time, (laserpar[8] > (dist_wall+0.4))&&(laserpar[8]!=0))) {
			printf("No more gate at laser: %f\n", laserpar[8]);
			mission.sub_state = 2;
			mission.sub_time = (-1);
		}
