...
const double linesensor_interp[2][8] = { {0.0343,0.0266,0.0625,0.0498,0.0714,0.0457,0.0226,0.0746},
				 {-2.4241,-1.7012,-3.4521,-2.9450, -3.9218,-2.6641,-1.5437,-4.1571} };
...
enum {ms_init,ms_fwd,ms_follow_line,ms_follow_white_line,ms_turn,ms_end,ms_goto_box,ms_measBox,ms_wait,ms_gate_loose,ms_gate_wall,ms_wait_2};

const double BLACK_THRESHOLD = 0.12;
const double WHITE_THRESHOLD = 0.75;
const double robot_length = 0.25;//0.28;
const double back_gate_dist=0.035;
...
/*  end of mission  */
  ...
  //printf("%f;%f;%d;%d\n",laserpar[0],laserpar[8],mission.sub_state,sens.cross);
...
int follow_wall(double speed, char dir, double dist_wall, int time, int condition) {
	...
	double dist = 0;
	if (dir == 'r') {
		if (laserpar[7] > dist_wall + 0.2 && laserpar[8] > dist_wall + 0.1) {
		...
		if (laserpar[1] > dist_wall + 0.2 && laserpar[0] > dist_wall + 0.1) {
			printf("no wall to follow");
			return 1;
		}
		else {
...
{    case 0:
          if(fwd(0.05,speed/2,mission.sub_time,0)){
            mission.sub_state = 1;
            mission.sub_time = (-1);
       printf("fwd 0.05\n"); }
      break;
     case 1:
         if (follow_line(speed/4,'c',0,mission.sub_time,laserpar[0]<1.1,followdist)){
     ...
     break;
     case 2:
          if (follow_line(speed/4,'c',0,mission.sub_time,laserpar[0]<0.5,followdist)){
     ...
     break;
     case 4:
         if (turn(white_line_turn, speed/3, mission.sub_time)){
            mission.sub_state = 5;
           mission.sub_time = (-1);}
     break;
     case 5:
     ...
     break;
     case 6:
         if (follow_line(speed/3,'c',0,mission.sub_time,(sens.linesensor[4] > WHITE_THRESHOLD)&&(sens.linesensor[3] > WHITE_THRESHOLD)&&(sens.linesensor[5] > WHITE_THRESHOLD)&&(sens.linesensor[6] > WHITE_THRESHOLD)&&(sens.linesensor[7] > WHITE_THRESHOLD)&&(sens.linesensor[0] > WHITE_THRESHOLD)&&(sens.linesensor[1] > WHITE_THRESHOLD)&&(sens.linesensor[2] > WHITE_THRESHOLD),followdist)){
             mission.sub_state = 7;
             mission.sub_time = (-1);}
     break;
     case 7:
          if (fwd(0.1,speed/3,mission.sub_time,0)){
           return 1;} 
}
            return 0;     
}
...
int goto_box(double speed,double followdist) {
	...
	case 3:
        ...
        case 8:
		if (fwd(0.3, 20, mission.sub_time, 0)) {
			return 1;
		}
		break;
        }
...
int gate_loose(double speed, int time,double followdist) {
	...
	{
	case 0:
		if (follow_line(speed / 2, 'r', 0, mission.sub_time, sens.cross,followdist)) {
			mission.sub_state = 1;
			mission.sub_time = (-1);
			dist_to_wall = 1.3;
			mot.flag_collision=0;
		...
		break;
	case 1:
		if (follow_line(speed / 2, 'c', 0, mission.sub_time, laserpar[0] < (dist_to_wall-0.3),followdist)) {
		...
		break;
	case 2:
		if (follow_line(speed / 2, 'c', 0, mission.sub_time, laserpar[0] >(dist_to_wall-0.3),followdist)) {//go forward no longer see gatepost
		...
		break;
	case 3:
		if (follow_line(speed / 2, 'c', 0, mission.sub_time, laserpar[0] < (dist_to_wall-0.3),followdist)) {//go forward see gatepost
			printf("gate!! %f\n", laserpar[0]);
			mission.sub_state = 4;
			mission.sub_time = (-1);
		}
	...
	case 6:
		...
		break;
	case 7:
		if (fwd(dist_to_wall - 0.1-robot_length, speed, mission.sub_time, 0)) {//go to 20 cm from wall
			mission.sub_state = 8;
			mission.sub_time = (-1);
		}
		break;
...
int gate_wall(double speed,double dist_wall, int time) {
	...
	{
	case 0:
		if (follow_wall(speed, 'l', dist_wall, mission.sub_time, laserpar[0] > (dist_wall+0.4))) {
			printf("No more gate at laser: %f\n", laserpar[0]);
			mission.sub_state = 2;
			mission.sub_time = (-1);
		}
      ...
      /*case 1:
		...
		break;*/
	case 2:
		if (fwd(1, speed, mission.sub_time, laserpar[0] < (dist_wall+0.4))) {
			printf("Gate found maybe at laser: %f?\n", laserpar[0]);
			mission.sub_state = 3;
			mission.sub_time = (-1);
		}
	...
	case 5:
		...
		break;
	case 6:
		if (fwd(2*dist_wall,speed, mission.sub_time, 0)) {
			mission.sub_state = 7;
			mission.sub_time = (-1);
		}
		break;
	...
	/*case 10:
		...
		break;*/
	case 11:
		if (fwd(1, speed, mission.sub_time, laserpar[0] < (dist_wall+0.2))) {
			printf("Gate found maybe at laser: %f?\n", laserpar[0]);
	...
	case 23:
		...
		break;
	case 24:
		if (follow_line(speed / 2, 'c', 0, mission.sub_time, sens.cross, 1)) {
