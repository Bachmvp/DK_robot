...
*/
   ...
   switch (mission.state) {
     ...
     case ms_follow_line:
       if (follow_line(speed_go/2, 'c', 0, mission.time, 0,follow_dist)) 
	...
	mission.state = ms_garage_into;
	 ...
	 case ms_gate_wall:
		 if (gate_wall(speed_go, 0.2, mission.time)) {
			 mission.state = ms_end;
		 }
	break;
     case ms_turn:
...
/*  end of mission  */
 ...
 //printf("%d;%f;%d\n",mission.sub_time,laserpar[0],mission.sub_state);
...
{    case 0:
      ...
      break;
     case 1:
         if (follow_line(speed/4,'c',1,mission.sub_time, (sens.linesensor[3] < BLACK_THRESHOLD) && (sens.linesensor[4] < BLACK_THRESHOLD),followdist)){
...
int gate_loose(double speed, int time,double followdist) {
	...
	{
	case 0:
		if (follow_line(speed / 4, 'c', 1, mission.sub_time, sens.w_cross,0.2)) {
			mission.sub_state = 1;
			mission.sub_time = (-1);
			dist_to_wall = 1.4;
			mot.flag_collision=0;
		...
		break;
	case 1:
		if (follow_line(speed / 4, 'c', 1, mission.sub_time, (laserpar[0] < (dist_to_wall-0.3))&&(laserpar[0]!=0),followdist)) {
		...
		break;
	case 2:
		if (follow_line(speed / 4, 'c', 1, mission.sub_time, (laserpar[0] >(dist_to_wall-0.3))&&(laserpar[0]!=0),followdist)) {//go forward no longer see gatepost
			printf("No more gate at laser: %f\n", laserpar[0]);
                        dist_to_wall=laserpar[0];
			mission.sub_state = 3;
			mission.sub_time = (-1);
		}
		break;
	case 3:
		if (follow_line(speed / 4, 'c', 1, mission.sub_time, (laserpar[0] < (dist_to_wall-0.3))&&(laserpar[0]!=0),followdist)) {//go forward see gatepost
			printf("gate!! %f\n", laserpar[0]);
			mission.sub_state = 4;
			mission.sub_time = (-1);
		}
		...
		break;
	case 5:
		if (fwd(back_gate_dist-0.01, -1 * speed, mission.sub_time, 0)) {
			mission.sub_state = 6;
			mission.sub_time = (-1);
		}
		break;
...
int gate_wall(double speed,double dist_wall, int time) {
	...
	{
	case 0:
		if (follow_wall(speed, 'l', dist_wall, mission.sub_time, (laserpar[0] > (dist_wall+0.4))&&(laserpar[0]!=0))) {
			printf("No more gate at laser: %f\n", laserpar[0]);
			mission.sub_state = 2;
			mission.sub_time = (-1);
		}
		...
		break;*/
	case 2:
		if (fwd(1, speed, mission.sub_time, (laserpar[0] < (dist_wall+0.4))&&(laserpar[0]!=0))) {
			printf("Gate found maybe at laser: %f?\n", laserpar[0]);
			mission.sub_state = 3;
			mission.sub_time = (-1);
		}
	...
	/*case 10:
		...
		break;*/
	case 11:
		if (fwd(1, speed, mission.sub_time, (laserpar[0] < (dist_wall+0.2))&&(laserpar[0]!=0))) {
			printf("Gate found maybe at laser: %f?\n", laserpar[0]);
	...
	case 12:
		if (fwd(1, speed, mission.sub_time, (laserpar[0] > (dist_wall+0.2))&&(laserpar[8]!=0))) {
			printf("No more gate at laser: %f\n", laserpar[0]);
...
int garage_into(double speed,double followdist){
	...
	case 0:
		if (follow_line(speed / 2, 'c', 0, mission.sub_time,laserpar[4] < 0.2,followdist)) {
			mission.sub_state = 1;
			mission.sub_time = (-1);
		}
		break;
...
void update_sensors(sensetype *s, odotype *q)
{
	...
	double Ka[5] = { 14.61, 14.11, 14.27, 14.44, 15.41 };
	double Kb[5] = { 48.09, 69.17, 76.33, 94.21, 67.15 };
